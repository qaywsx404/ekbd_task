...
CREATE OR REPLACE TRIGGER konkurs_mdate_update BEFORE UPDATE ON ebd_ekbd.deposit
    FOR EACH ROW
    EXECUTE PROCEDURE ebd_ekbd.f_update_mdate();

		-- для Нефтегазоперспективные структуры
CREATE OR REPLACE TRIGGER konkurs_mdate_update BEFORE UPDATE ON ebd_ekbd.struct
    FOR EACH ROW
    EXECUTE PROCEDURE ebd_ekbd.f_update_mdate();

			-- для Особо охраняемые территории (заповедники) 
CREATE OR REPLACE TRIGGER konkurs_mdate_update BEFORE UPDATE ON ebd_ekbd.zapovednik
    FOR EACH ROW
    EXECUTE PROCEDURE ebd_ekbd.f_update_mdate();

				-- для НГО
CREATE OR REPLACE TRIGGER konkurs_mdate_update BEFORE UPDATE ON ebd_ekbd.ngo
    FOR EACH ROW
    EXECUTE PROCEDURE ebd_ekbd.f_update_mdate();

					-- для НГП
CREATE OR REPLACE TRIGGER konkurs_mdate_update BEFORE UPDATE ON ebd_ekbd.ngp
    FOR EACH ROW
    EXECUTE PROCEDURE ebd_ekbd.f_update_mdate();

					-- для НГР
CREATE OR REPLACE TRIGGER konkurs_mdate_update BEFORE UPDATE ON ebd_ekbd.ngr
    FOR EACH ROW
    EXECUTE PROCEDURE ebd_ekbd.f_update_mdate();
